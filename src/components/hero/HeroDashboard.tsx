    ...
    >
      ...
      <motion.div style={{ y: yTabs }} className="absolute top-0 md:top-4 z-40 w-full flex justify-center pointer-events-none">
        <motion.div 
          ...
          className="flex items-center gap-1 sm:gap-2 p-1.5 bg-bg-base/70 dark:bg-[#0D1424]/80 backdrop-blur-2xl border border-border-main rounded-full shadow-glass pointer-events-auto transition-all duration-400"
        >
          {TABS.map((tab, idx) => (
            <button
              key={tab.id}
        ...
        <div className="absolute top-0 w-[520px] md:w-[580px] lg:w-[600px] h-[400px] scale-[0.65] sm:scale-[0.85] md:scale-100 origin-top perspective-[1400px]">
              ...
              <div className="absolute top-0 left-0 w-full h-10 bg-bg-subtle border-b border-border-main flex items-center px-3 justify-between z-20 transition-colors duration-400">
                ...
                {activeTab === 4 && (
                  <motion.div 
                    ...
                    className="absolute inset-0 pt-10 flex flex-col bg-bg-base overflow-hidden transition-colors duration-400"
                  >
                    <div className="px-4 py-2 flex justify-between items-center border-b border-border-main bg-bg-subtle/50 z-10 transition-colors duration-400">
                      <div>
                        <h2 className="text-[11px] font-black text-text-primary tracking-tight">Island-Wide Network</h2>
                        <p className="text-text-secondary text-[8px] font-medium mt-0.5">Real-time sync across all 9 provinces</p>
                      </div>
                      <div className="flex items-center gap-1 bg-success/10 text-success px-2 py-0.5 rounded-md font-bold text-[8px] border border-success/20">
                        <div className="w-1 h-1 rounded-full bg-success animate-pulse"></div> Live Nodes
                      </div>
                    </div>

                    <div className="flex-1 relative flex">
                      <div className="w-1/3 border-r border-border-main bg-bg-subtle/80 backdrop-blur-md z-10 flex flex-col p-3 gap-3 transition-colors duration-400">
                        <div>
                          <div className="text-[7px] font-bold text-text-muted uppercase tracking-widest mb-0.5">Active Tenants</div>
                          <div className="text-sm font-black text-text-primary">1,500+</div>
                        </div>
                        <div>
                          <div className="text-[7px] font-bold text-text-muted uppercase tracking-widest mb-0.5">Data Latency</div>
                          <div className="text-xs font-bold text-success">{'< 50ms'}</div>
                        </div>
                        <div className="flex-1"></div>
                        <div className="flex items-center gap-1.5 text-[7px] text-text-secondary">
                          <Activity size={10} className="text-blue-primary" />
                          <span>Syncing isolated DBs...</span>
                        </div>
                      </div>

                      <div className="flex-1 relative bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-opacity-5 dark:bg-opacity-10">
                        <div className="absolute inset-0 bg-gradient-to-tr from-blue-primary/10 to-transparent"></div>
                        ...
                        {MINI_REGIONS.map((region, i) => (
                          <div key={region.name} className="absolute flex flex-col items-center" style={{ top: region.y - 12, left: region.x - 18 }}>
                            <div className="relative flex h-1.5 w-1.5">
                              ...
                              <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-blue-primary shadow-[0_0_10px_var(--color-blue-primary)]"></span>
                            </div>
                            <span className="text-[6px] font-bold text-text-primary dark:text-slate-300 mt-0.5 uppercase tracking-widest">{region.name}</span>
                          </div>
                        ))}

                        <svg className="absolute inset-0 w-full h-full opacity-30" viewBox="0 0 350 250">
                          <motion.path d="M 140 20 L 155 135 L 140 215" stroke="var(--color-blue-primary)" strokeWidth="1" strokeDasharray="2 2" fill="none" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 3, repeat: Infinity }} />
                          <motion.path d="M 70 165 L 155 135 L 230 95" stroke="var(--color-success)" strokeWidth="1" strokeDasharray="2 2" fill="none" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 2.5, repeat: Infinity, delay: 0.5 }} />
                        </svg>
                      </div>
                    </div>
                  </motion.div>
          ...
          <motion.div style={{ y: isReducedMotion ? 0 : yBanner }} className="absolute inset-0 z-30 pointer-events-none">
            <motion.div 
              ...
              className="absolute -top-5 left-8 bg-amber-500/10 border border-amber-500/20 backdrop-blur-md px-4 py-2 rounded-full shadow-sm flex items-center gap-2 cursor-pointer hover:bg-amber-500/20 transition-all duration-300 pointer-events-auto"
            >
              <span className="text-[10px] font-bold text-amber-700 dark:text-amber-300">⚡ Your trial ends in 14 days. Upgrade Now →</span>
            </motion.div>
          </motion.div>
